...
router.put("/:employeeId" , parseAuthHeaderMiddleware(), parseAuthorizationMiddleware(),requiredPermission(["update_employee"]), validate(updateEmployeeSchema) ,EmployeeController.updateEmployeeController)
router.delete("/:employeeId" , parseAuthHeaderMiddleware(),parseAuthorizationMiddleware(), requiredPermission(["delete_employee"]) , EmployeeController.deleteEmployeeController)

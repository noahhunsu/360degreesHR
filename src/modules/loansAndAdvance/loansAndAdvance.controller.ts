...
import { JobApplicationService } from "./loansAndAdvance.service.js";
...
export class JobApplicationController {
  static async createJobApplicationController(
    ...
    try {
      ...
      const result = await JobApplicationService.createJobApplicationService(
        req.body,
        jobOpeningId,
      );
  ...
  static async getSingleJobApplicationController(
    ...
    try {
      ...
      const result = await JobApplicationService.getSingleJobApplicationService(
        user,
        applicationId,
      );
      return res.status(200).json({
  ...
  static async moveJobApplicationController(
    ...
    try {
      ...
      const result = await JobApplicationService.moveJobApplicationService(
        user,
        payload,
      );
      return res.status(200).json({
  ...
  static async rejectJobApplicationController(
    ...
    try {
      ...
      const result = await JobApplicationService.rejectJobApplicationService(
        user,
        payload,
      );
      return res.status(200).json({
